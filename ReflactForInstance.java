...
public class ReflactForInstance {
	...
	public static <T> T reflactToEntity(Class<T> cls, Object object, Map<String,String> values) throws Exception
	{
		String fieldName = "";
			...
			for(Field field : fields){
				...
				for(String key : set){
					if(fieldName.toLowerCase().equals(key.toLowerCase())){
						...
						try{
							fieldSetMethod = model.getDeclaredMethod("set" + convertFirstCharToUpper(key), fieldType);
						}catch(Exception e){
							errorMsg = String.format("字段 ["+key+"] 转换set方法失败");
						...
						try{
							...
							logger.error(e.getMessage());
							errorMsg = String.format("Error:数据类型转换失败，columnName=【"+fieldName+"】，fieldType=【"+fieldType.getName()+"】，fieldValue=【"+fieldValue+"】 messge：【%s】", e.getMessage());
							throw new Exception();
						}
						break;
					}
