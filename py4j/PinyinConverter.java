...
public class PinyinConverter implements Converter {
	...
	@Override
	public String getSimplePinyin(String chinese) throws IllegalPinyinException {
		if(StringUtils.isEmpty(chinese)){
			return null;
		}
		...
		for(int i=0;i<chs.length;i++){
			...
			if(py_arr.length==1){
				py_sb.append(getFirstLetterToUpperCase(py_arr[0]));
			}else if(py_arr.length==2 && py_arr[0].equals(py_arr[1])){
				py_sb.append(getFirstLetterToUpperCase(py_arr[0]));
			}else{
				String resultPy = null, defaultPy = null;;
				...
				py_sb.append(getFirstLetterToUpperCase(resultPy));
			}
		}
	...
	private String getFirstLetterToUpperCase(String str) {
		if (str == null || str.length()==0) {
			return "";
		}
